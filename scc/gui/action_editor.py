		...
		if isinstance(action, InvalidAction):
			...
			if isinstance(action, ModeModifier):
				for k in action.mods:
					if action.mods[k]:
						action.mods[k] = self.generate_modifiers(ActionEditor.strip_modifiers(action.mods[k]))
				if action.default:
					action.default = self.generate_modifiers(ActionEditor.strip_modifiers(action.default))
			else:
