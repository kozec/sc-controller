...
from scc.tools import strip_gesture, nameof, clamp, encode_escape
...
class ChangeProfileAction(Action, SpecialAction):
	...
	def to_string(self, multiline=False, pad=0):
		return (" " * pad) + "%s('%s')" % (self.COMMAND,
				encode_escape(self.profile.encode('utf-8')))
...
class OSDAction(Action, SpecialAction):
	...
	def to_string(self, multiline=False, pad=0):
		...
		if self.action:
			...
			parameters.append("'%s'" % (encode_escape(str(self.text).encode('utf-8')),))
...
class DialogAction(Action, SpecialAction):
	...
	def to_string(self, multiline=False, pad=0):
		...
		rv += "'%s', " % (encode_escape(self.text.encode('utf-8')),)
		if multiline:
			rv += "\n%s" % (" " * (pad + 2))
