...
class ControllerImage(SVGWidget):
	...
	def _ensure_config(self, data, controller):
		...
		if not data['gui'].get("background", None):
			data['gui']['background'] = "sc"
			if controller and controller.get_type():
				type_config = os.path.join(get_share_path(), "%s.config.json" % (controller.get_type(),))
				if os.path.exists(type_config):
					default_data = json.loads(open(type_config, "r").read())
					data['gui']['background'] = default_data['gui']['background']
		else:
			data['gui']['background'] = data['gui']["background"]
		data['gui']['buttons'] = (data['gui'].get("buttons")
			or default_data.get("gui", {}).get("buttons")
			or self._get_default_images()
		)
		data['buttons'] = (data.get("buttons")
			or default_data.get("buttons")
			or ControllerImage.DEFAULT_BUTTONS
		)
		data['axes'] = (
			data.get("axes")
			or default_data.get("axes")
			or ControllerImage.DEFAULT_AXES
		)
		data['gyros'] = data.get("gyros", data['gui']["background"] in ("sc", "deck"))
		return data
